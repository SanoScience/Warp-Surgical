...
class HapticProxyState:
    """GPU-resident haptic sphere state shared across runtimes."""
...
def create_haptic_proxy_state(
    *,
    ...
    """Allocate the persistent GPU buffers used to drive a kinematic sphere."""

    raw = _as_vec3f(initial_position) if initial_position is not None else wp.vec3f(0.0, 0.0, 0.0)
    scaled = (
        _as_vec3f(initial_scaled_position)
        if initial_scaled_position is not None
        else wp.vec3f(0.0, 0.0, 0.0)
    )
...
def create_vec3_staging_buffer():
    """Return a reusable CPU staging buffer for one vec3 sample."""
