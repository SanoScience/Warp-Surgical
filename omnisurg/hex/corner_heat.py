...
"""Host-side cell-centred heat driver for corner-grid diathermy."""
...
from .corner_delete import CornerDeletionState, DeviceDeletionResult
from .corner_grid import CornerGridAuxState
from .kernels.corner_heat import (
...
@dataclass
class CornerHeatState:
    """Mutable per-cell heat buffers for the corner-node lattice."""
    ...
    def select_overheated_cells(self, material_cuttable: wp.array | None = None) -> CornerHeatSelection:
    ...
    def delete_overheated_cells_async(
        self,
        delete_state: CornerDeletionState,
        material_cuttable: wp.array | None = None,
    ) -> DeviceDeletionResult:
        """Select overheated cells and commit them through ``CornerDeletionState``."""
    ...
    def reset_deleted_cells(self) -> None:
        """Clear heat/burn on cells already removed from the corner grid."""
        wp.launch(
            reset_deleted_cell_heat_kernel,
            dim=int(self.aux.num_cells),
            inputs=[
    ...
    def update_heat_overlay(
        ...
        *,
        ...
        if hidden:
            viewer.log_points(name="/corner_grid/heat", points=None, hidden=True)
            return 0
...
def make_corner_heat_state(
    model: newton.Model,
    aux: CornerGridAuxState,
    fulguration: float = 0.05,
) -> CornerHeatState:
    """Build a :class:`CornerHeatState` from a corner grid."""
    device = model.device
    mats = aux.materials
    return CornerHeatState(
        model=model,
        aux=aux,
