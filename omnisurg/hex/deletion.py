...
"""Host-side voxel deletion for the springless hex particle lattice."""
...
from .hex_grid import HexGridAuxState, ShapeMatchingClusters
from .kernels.deletion import (
...
class HexDeletionState:
    ...
    def __post_init__(self) -> None:
        if self.model.spring_count != 0:
            raise ValueError(f"HexDeletionState requires a springless model; got spring_count={self.model.spring_count}")
    ...
    def capture_reset_baseline(self) -> None:
        ...
        ``HexDeletionState`` is often constructed before shape-matching
    ...
    def reset(self) -> None:
        ...
        self._refresh_shape_matching_coefficients()
    ...
    def _sync_all_cluster_hosts(self) -> None:
        ...
        self._sync_cluster_host(hierarchy.l2_full125)

    def _normalize_sync_domains(self, domains) -> set[str]:
        if domains == "all" or domains is None:
            return {"cells", "nodes", "mass", "clusters", "stats", "last-delete"}
        ...
        for domain in raw:
            if domain in {"all", "*"}:
                normalized.update({"cells", "nodes", "mass", "clusters", "stats", "last-delete"})
    ...
    def _refresh_cluster_coefficients(self, clusters: ShapeMatchingClusters | None) -> None:
        if clusters is None:
            return
        cell_to_cluster = clusters.cell_to_cluster_host.astype(np.int32, copy=False)
        valid_cells = np.flatnonzero((cell_to_cluster >= 0) & (self.cell_active != 0)).astype(np.int32, copy=False)
        coefficients = np.zeros(clusters.num_clusters, dtype=np.float32)
        if valid_cells.size > 0:
            cluster_ids = cell_to_cluster[valid_cells]
            cell_material = self.aux.cell_material_host[valid_cells]
            scales = self.material_stiffness_scale[cell_material].astype(np.float32, copy=False)
            sums = np.bincount(cluster_ids, weights=scales, minlength=clusters.num_clusters).astype(np.float32)
            counts = np.bincount(cluster_ids, minlength=clusters.num_clusters).astype(np.float32)
            has_cells = counts > 0
            coefficients[has_cells] = clusters.base_coefficients_host[has_cells] * (sums[has_cells] / counts[has_cells])
        clusters.coefficients_host[:] = coefficients
        clusters.coefficients.assign(clusters.coefficients_host)

    def _refresh_shape_matching_coefficients(self) -> None:
        for clusters in self._iter_shape_matching_clusters():
            self._refresh_cluster_coefficients(clusters)
    ...
    def _launch_single_delete_gpu_from_device(self, cell_ids: wp.array) -> DeviceDeletionResult:
        ...
        self._mark_dirty("cells", "nodes", "mass", "clusters", "last-delete", "stats")
        return result
    ...
    def set_material_stiffness_scale(self, values) -> None:
        """Update per-material multipliers and push shape-matching coefficients."""
    ...
    def delete_cells(self, cell_indices: np.ndarray | list[int]) -> int:
        """Deactivate specific cells and update shared particle state."""
        previous_revision = int(self.topology_revision)
    ...
    def validate_device_state(self) -> None:
        ...
        for clusters in (
            ...
            None if self.aux.shape_matching_hierarchy is None else self.aux.shape_matching_hierarchy.l2_full125,
        ):
            if clusters is None:
                continue
            wp.launch(
