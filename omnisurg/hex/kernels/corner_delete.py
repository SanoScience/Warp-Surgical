...
"""Sparse GPU updates for corner-grid cell deletion."""
...
_CELL_CONSTRAINTS = wp.constant(28)
...
def delete_cells_sparse_kernel(
    ...
    cell_constraint_offsets: wp.array(dtype=wp.int32),
    cell_constraint_indices: wp.array(dtype=wp.int32),
    cell_active: wp.array(dtype=wp.int32),
    node_support_count: wp.array(dtype=wp.int32),
    node_mass: wp.array(dtype=float),
    spring_support_count: wp.array(dtype=wp.int32),
    ...
    derived node flags / inverse masses and spring stiffnesses are refreshed by
    the follow-up kernels after all atomics are complete.
    """
    tid = wp.tid()
    ...
    start = cell_constraint_offsets[cell_idx]
    end = cell_constraint_offsets[cell_idx + 1]
    cursor = start
    while cursor < end:
        spring_idx = cell_constraint_indices[cursor]
        wp.atomic_sub(spring_support_count, spring_idx, 1)
        cursor += 1
...
def delete_single_cell_complete_kernel(
    ...
    node_material: wp.array(dtype=wp.int32),
    material_stiffness_scale: wp.array(dtype=float),
    spring_unit_stiffness: wp.array(dtype=float),
    spring_unit_damping: wp.array(dtype=float),
    ...
    while cursor < end:
        spring_idx = cell_constraint_indices[cursor]
        wp.atomic_sub(spring_support_count, spring_idx, 1)
        support = spring_support_count[spring_idx]
        if support <= 0:
            spring_enabled[spring_idx] = 0
            spring_stiffness[spring_idx] = 0.0
            spring_damping[spring_idx] = 0.0
        else:
            i = spring_indices[spring_idx * 2 + 0]
            j = spring_indices[spring_idx * 2 + 1]
            scale = 0.5 * (
                material_stiffness_scale[node_material[i]]
                + material_stiffness_scale[node_material[j]]
            )
            spring_enabled[spring_idx] = 1
            spring_stiffness[spring_idx] = spring_unit_stiffness[spring_idx] * float(support) * scale
            spring_damping[spring_idx] = spring_unit_damping[spring_idx] * float(support)
        cursor += 1
...
def finalize_deleted_cell_nodes_kernel(
    ...
    """Refresh derived node state for the corner nodes touched by deleted cells."""
...
@wp.kernel
def finalize_deleted_cell_springs_kernel(
    deleted_cells: wp.array(dtype=wp.int32),
    deleted_count: wp.array(dtype=wp.int32),
    num_cells: int,
    cell_constraint_offsets: wp.array(dtype=wp.int32),
    ...
    """Refresh live spring values for constraints touched by deleted cells."""
    cell_slot, local_constraint = wp.tid()
    if cell_slot >= deleted_count[0]:
        return
    ...
    cursor = start + local_constraint
    if cursor >= end or local_constraint >= _CELL_CONSTRAINTS:
        return
...
@wp.kernel
def validate_spring_state_kernel(
    spring_support_count: wp.array(dtype=wp.int32),
    spring_enabled: wp.array(dtype=wp.int32),
    spring_stiffness: wp.array(dtype=float),
    spring_damping: wp.array(dtype=float),
    ...
    i = wp.tid()
    support = spring_support_count[i]
    enabled = spring_enabled[i]
    stiffness = spring_stiffness[i]
    damping = spring_damping[i]
    ...
    if enabled != 0 and enabled != 1:
        bad = 1
    if support <= 0 and enabled != 0:
        bad = 1
    if support > 0 and enabled == 0:
        bad = 1
    if stiffness < -1.0e-5 or damping < -1.0e-5:
        bad = 1
    if stiffness != stiffness or damping != damping:
        bad = 1

    if bad != 0:
        old = wp.atomic_add(error_count, 0, 1)
        if old == 0:
            first_error[0] = 3000 + i
...
def select_sphere_particle_contact_cells_kernel(
    ...
    """Append active cells whose corner particles intersect an enabled sphere."""
    c = wp.tid()
    if c >= num_cells:
        return
    if cell_active[c] == 0:
