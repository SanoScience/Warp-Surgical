...
"""GPU-resident render layer for the corner-node cell lattice.

The physics lives on voxel corners (26-neighbour topology) but the existing
...
* ``cell_center_q`` - mean of the 8 corner positions
* ``cell_orientation`` - intrinsic mat33 from averaged corner edge vectors
* ``cell_render_flags`` - ``ParticleFlags.ACTIVE`` iff ``cell_active != 0``
* ``cell_stretch`` - max absolute strain over the cell's 28 physics constraints

Render occupancy keys off ``cell_active``, not the node ACTIVE flag: a deleted
cell can leave all 8 corner nodes still ACTIVE via support from neighbours, so
...
def update_cell_render_state_kernel(
    ...
    Corner ordering matches ``CELL_CORNER_OFFSETS`` in corner_grid.py:
    ``0:(0,0,0) 1:(1,0,0) 2:(1,1,0) 3:(0,1,0) 4:(0,0,1) 5:(1,0,1) 6:(1,1,1) 7:(0,1,1)``.

    The frame axes come from averaged corner-pair deltas:
...
def update_cell_render_state(
    ...
    """Launch the per-frame render-state update. ``aux`` is a CornerGridAuxState."""
    wp.launch(
