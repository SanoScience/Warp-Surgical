...
"""Cell-centred heat and instrument selection kernels for hex grids."""
...
def _cell_intersects_sphere(
    ...
    for local_node in range(8):
        ...
        particle_delta = particle_q[node_idx] - sphere_centre
        if wp.dot(particle_delta, particle_delta) <= r2:
            return 1
    return 0
...
def select_capsule_cells_kernel(
    ...
    """Append active cuttable cells whose centre or particles intersect a capsule."""
    c = wp.tid()
    if c >= num_cells:
        return
    if cell_active[c] == 0:
