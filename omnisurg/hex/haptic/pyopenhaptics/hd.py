...
def get_velocity() -> hduVector3Dd:
    return _get_doublev(HD_CURRENT_VELOCITY, hduVector3Dd)
...
def get_error() -> bool:
    ...
    if error not in exception_dict:
        if error == HD_SUCCESS:
            return False
        print(f"HD error 0x{error:04x}: Error during the main scheduler.")
        return True
    try:
        if error == HD_SUCCESS:
            return False
        ...
        print(f"{type(e).__name__}: Error during the main scheduler.")
        return True
...
def unschedule(handle: int) -> None:
    _lib_hd.hdUnschedule.argtypes = [HDSchedulerHandle]
