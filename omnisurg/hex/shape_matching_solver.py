...
"""Particle-only hex-grid solver with shape matching."""
...
from .hex_grid import HierarchicalShapeMatchingClusters, ShapeMatchingClusters
...
class HexShapeMatchingSolver(SolverBase):
    """Local particle solver for springless hex grids with shape matching."""
    ...
    def __init__(
        ...
        *,
        ...
        if model.body_count != 0:
            raise ValueError(f"HexShapeMatchingSolver only supports particle-only models; got body_count={model.body_count}")
        if model.shape_count != 0:
            raise ValueError(
                f"HexShapeMatchingSolver does not support rigid/shape contacts in v1; got shape_count={model.shape_count}"
            )
        if model.edge_count != 0 or model.tet_count != 0 or model.tri_count != 0 or model.joint_count != 0:
            raise ValueError("HexShapeMatchingSolver only supports particles, self-collision, and shape-matching clusters")
        if model.spring_count != 0:
            raise ValueError(f"HexShapeMatchingSolver requires a springless model; got spring_count={model.spring_count}")
    ...
    def step(
        ...
        for _ in range(self.iterations):
            ...
            base_delta_active = bool(self_collision_active or ground_active)
