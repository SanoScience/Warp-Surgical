...
"""Digimouse corner-grid viewer with shape matching, MC surface, and mouse drag.

Combines the full Digimouse marching-cubes viewer from example 08 with the
local particle-only spring + shape-matching solver from example 09.
...
* per-family spring stiffness plus shape-matching controls
...
from omnisurg.hex.corner_delete import DeviceDeletionResult, make_corner_deletion_state  # noqa: E402
from omnisurg.hex.corner_heat import make_corner_heat_state  # noqa: E402
from omnisurg.hex.corner_grid import (  # noqa: E402
    build_corner_grid,
    build_corner_l1_shape_matching_clusters,
    build_corner_shape_matching_clusters,
)
from omnisurg.hex.corner_solver import (  # noqa: E402
...
from omnisurg.hex.kernels.corner_render import HoverPicker, update_cell_render_state  # noqa: E402
...
def _crop_corner_atlas_to_visible_classes(
...
class UiState:
    ...
    edge_ke: float = 1800.0
    face_ke: float = 1800.0
    body_ke: float = 1800.0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=("diathermy", "diathermy"),
        metavar=("FIRST", "SECOND"),
        help="per-instrument mode for the two input devices; deprecated cutting aliases to diathermy.",
    )
    ...
    parser.add_argument("--edge-ke", type=float, default=1800.0)
    parser.add_argument("--edge-kd", type=float, default=2.0)
    parser.add_argument("--face-ke", type=float, default=1800.0)
    parser.add_argument("--face-kd", type=float, default=2.0)
    parser.add_argument("--body-ke", type=float, default=1800.0)
    parser.add_argument("--body-kd", type=float, default=2.0)
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="apply L1 Outer8 coarse-corner corrections to skipped fine nodes by trilinear prolongation",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="apply L2 Outer8 coarse-corner corrections to skipped fine nodes by trilinear prolongation",
    )
    parser.add_argument(
        "--hierarchical-shape-matching-outer8-absolute-projection",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="set L1/L2 Outer8 prolonged children to absolute trilinear corner positions instead of applying corner deltas",
    )
    ...
    parser.add_argument("--spring-width", type=float, default=0.00035)
    parser.add_argument(
        "--cryo-texture",
        type=str,
    ...
    with _StartupPhase("atlas_load", startup_phases):
        if _OMNISURG_PREPARED_VOLUME is not None:
            atlas = _OMNISURG_PREPARED_VOLUME.to_corner_atlas()
        ...
        if _OMNISURG_PREPARED_VOLUME is None and args.crop_visible_classes is not None:
            atlas, visible_class_crop = _crop_corner_atlas_to_visible_classes(
                atlas,
    ...
    with _StartupPhase("build_corner_grid", startup_phases):
        pg = build_corner_grid(
            atlas,
            edge_ke=args.edge_ke,
            edge_kd=args.edge_kd,
            face_ke=args.face_ke,
            face_kd=args.face_kd,
            body_ke=args.body_ke,
    ...
    with _StartupPhase("make_corner_deletion_state", startup_phases, sync_device=dev):
        delete_state = make_corner_deletion_state(model, pg.aux)
    with _StartupPhase("make_corner_heat_state", startup_phases, sync_device=dev):
        heat_state = make_corner_heat_state(model, pg.aux)
    ...
    ui = UiState(
        ...
        face_ke=float(args.face_ke),
        body_ke=float(args.body_ke),
        shape_matching_stiffness=float(args.shape_matching_stiffness),
        shape_matching_relaxation=float(args.shape_matching_relaxation),
        shape_matching_passes=int(args.shape_matching_passes),
        shape_matching_mode=shape_matching_mode,
    ...
    delete_state.set_family_stiffnesses(edge=ui.edge_ke, face=ui.face_ke, body=ui.body_ke)
        ...
        for idx, input_device in enumerate(input_devices):
            ...
            if pose.valid:
                ...
                if instrument_backend == "minimou":
                    trigger_down = float(instrument_tool_pos_host[idx]) < _INSTRUMENT_TRIGGER_THRESHOLD
                else:
    ...
    def _print_instrument_tool_positions() -> None:
        ...
        print(f"[instrument] tool_pos {values}", flush=True)
    ...
    def _apply_slang_environment_to_viewer() -> None:
        if not slang_viewer_requested:
            return
        if hasattr(viewer, "set_environment_path"):
            viewer.set_environment_path(ui.slang_environment_map or None)
        if hasattr(viewer, "set_environment_background_enabled"):
            viewer.set_environment_background_enabled(ui.slang_environment_background)
        if hasattr(viewer, "set_environment_intensity"):
            viewer.set_environment_intensity(ui.slang_environment_intensity)
        if hasattr(viewer, "set_environment_rotation_degrees"):
            viewer.set_environment_rotation_degrees(ui.slang_environment_rotation_degrees)
        if hasattr(viewer, "set_environment_pitch_degrees"):
            viewer.set_environment_pitch_degrees(ui.slang_environment_pitch_degrees)
    ...
    with _StartupPhase("overlays_and_picker", startup_phases, sync_device=dev):
        surface = SurfaceRenderer("corner_shape_matching_surface", n_cells, device=dev, max_triangles=mc_buffers.max_triangles)
        cell_overlay = ColoredParticleOverlay(n_cells, pg.aux.materials, device=dev, name="/model/particles")
        node_overlay = ColoredParticleOverlay(n_nodes, pg.aux.materials, device=dev, name="/corner_grid/nodes")
        spring_overlay = SpringOverlay(pg.aux.num_springs, device=dev, name="/corner_grid/springs")
    ...
    if _viewer_supports_mouse_interaction(viewer):
        ...
        if pyglet is not None:
            ...
            def _on_mouse_press(x, y, button, modifiers):
                ...
                if button == pyglet.window.mouse.MIDDLE:
                    if _plane_cut_modifier_down() or bool(modifiers & pyglet.window.key.MOD_SHIFT):
                        ray_origin, ray_direction = _mouse_world_ray(viewer, x, y)
                ...
                if button == pyglet.window.mouse.MIDDLE:
                    if bool(mouse_state["plane_cut_active"]):
                        ...
                        if start_origin is not None and start_direction is not None and depth > 0.0:
                            end_origin, end_direction = _mouse_world_ray(viewer, x, y)
            ...
            viewer.renderer.register_mouse_motion(_on_mouse_motion)
            viewer.renderer.register_mouse_press(_on_mouse_press)
            viewer.renderer.register_mouse_drag(_on_mouse_drag)
            viewer.renderer.register_mouse_release(_on_mouse_release)
        ...
        def render_ui(imgui):
            ...
            _, ui.show_nodes = imgui.checkbox("Show corner nodes", ui.show_nodes)
            _, ui.show_springs = imgui.checkbox("Show spring overlay", ui.show_springs)
            if slang_viewer_requested:
            ...
            _, ui.enable_springs = imgui.checkbox("Enable springs", ui.enable_springs)
            ...
            if instrument_backend == "minimou":
                imgui.text(
                    "MiniMou cut values: "
                    f"{instrument_tool_pos_host[0]:.2f}, {instrument_tool_pos_host[1]:.2f} "
                    f"(trigger < {_INSTRUMENT_TRIGGER_THRESHOLD:.1f})"
                )
            imgui.text(
                "Tool triggers: "
            ...
            imgui.separator()
            imgui.text("Constraint Stiffness")
            changed, new_edge = imgui.slider_float("Edge stiffness", ui.edge_ke, 0.0, 20000.0)
            if changed:
                ui.edge_ke = float(new_edge)
                ui.stiffness_dirty = True
            changed, new_face = imgui.slider_float("Face stiffness", ui.face_ke, 0.0, 20000.0)
            if changed:
                ui.face_ke = float(new_face)
                ui.stiffness_dirty = True
            changed, new_body = imgui.slider_float("Body stiffness", ui.body_ke, 0.0, 20000.0)
            if changed:
                ui.body_ke = float(new_body)
                ui.stiffness_dirty = True

        viewer.register_ui_callback(render_ui, position="side")
        ...
        viewer.register_ui_callback(render_shape_matching_panel, position="free")
        ...
        viewer.register_ui_callback(render_materials_panel, position="free")
        ...
        if slang_viewer_requested:
            ...
            viewer.register_ui_callback(render_lighting_panel, position="free")
            ...
            viewer.register_ui_callback(render_timing_panel, position="free")

    print(
        f"{scene_label}: nodes={n_nodes}, cells={n_cells}, springs={pg.aux.num_springs}, "
    ...
    def _diathermy_power_fraction(idx: int) -> float:
        ...
        if instrument_backend == "minimou":
            if int(instrument_pose_valid_host[idx]) == 0:
                return 0.0
            return float(1.0 - np.clip(float(instrument_tool_pos_host[idx]), 0.0, 1.0))
    ...
    def _reset_simulation() -> None:
        ...
        if ui.stiffness_dirty:
            delete_state.set_family_stiffnesses(edge=ui.edge_ke, face=ui.face_ke, body=ui.body_ke)
            ui.stiffness_dirty = False
    ...
    while args.frames is None or frame < args.frames:
        with _scoped_timer("frame"):
            ...
            if drag_start_xy is not None and _viewer_supports_mouse_interaction(viewer):
                ray_origin, ray_direction = _mouse_world_ray(viewer, drag_start_xy[0], drag_start_xy[1])
                particle_q_np = state_0.particle_q.numpy()
            ...
            if drag_count > 0 and _viewer_supports_mouse_interaction(viewer):
                if drag_xy is not None:
                    ray_origin, ray_direction = _mouse_world_ray(viewer, drag_xy[0], drag_xy[1])
            ...
            if cut_xy is not None and _viewer_supports_mouse_interaction(viewer):
                ...
                with _scoped_timer(timer_name):
                    with _scoped_timer("cut.ray_build"):
                        ray_origin_np, ray_direction_np = _mouse_world_ray(viewer, cut_xy[0], cut_xy[1])
            ...
            solver.enable_springs = bool(ui.enable_springs)
                    ...
                    if use_graph and not drag_active and not _instrument_mc_triangle_collision_active():
                        if graph_capture_key != graph_key:
                            ...
                            print(
                                ...
                                f"springs={'ON' if ui.enable_springs else 'OFF'} "
            ...
            if isinstance(viewer, newton.viewer.ViewerGL):
                with _scoped_timer("grab_overlay"):
                    ...
                    grab_line_width = args.spring_width * 2.5
            ...
            with _scoped_timer("spring_overlay"):
                spring_overlay.update(
                    viewer=viewer,
                    spring_indices=model.spring_indices,
                    particle_q=state_0.particle_q,
                    particle_flags=model.particle_flags,
                    stiffness=model.spring_stiffness,
                    hidden=not ui.show_springs,
                    color=(0.3, 0.75, 1.0),
                    width=args.spring_width,
                )
