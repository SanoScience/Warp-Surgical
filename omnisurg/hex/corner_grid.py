...
"""Corner-node voxel lattice with per-cell distance constraints.

This is an alternative to :mod:`omnisurg.hex.grid` for experiments where
voxels are deleted as whole cells rather than cut along individual edges.

Topology:

* particles live on voxel corners (shared grid nodes)
* occupied voxels become active cells that reference 8 corner nodes
* each cell contributes 28 distance constraints:
  12 edges, 12 face diagonals, 4 body diagonals

Constraints are deduplicated globally by node pair for solver efficiency, but
their stiffness and damping are scaled by the number of currently active owner
cells so the mechanics still reflect "one stencil per cell".
"""
...
CONSTRAINT_EDGE = 0
CONSTRAINT_FACE_DIAGONAL = 1
CONSTRAINT_BODY_DIAGONAL = 2

CELL_EDGE_PAIRS = np.asarray(
    [
        (0, 1), (1, 2), (2, 3), (3, 0),
        (4, 5), (5, 6), (6, 7), (7, 4),
        (0, 4), (1, 5), (2, 6), (3, 7),
...
CELL_FACE_DIAGONAL_PAIRS = np.asarray(
    [
        (0, 2), (1, 3),
        (4, 6), (5, 7),
        (0, 5), (1, 4),
...
CELL_BODY_DIAGONAL_PAIRS = np.asarray(
    [
        (0, 6), (1, 7), (2, 4), (3, 5),
    ],
    dtype=np.int32,
...
class ShapeMatchingClusters:
    """Cluster sidecar for corner-grid shape matching."""
...
class HierarchicalShapeMatchingClusters:
    """Optional coarse shape-matching data for corner-grid experiments."""
...
@dataclass
class CornerGridAuxState:
    """Auxiliary arrays and host-side topology for the corner-node lattice."""
    ...
    spring_support_count: wp.array  # int32[num_springs]
    spring_owner_count: wp.array  # int32[num_springs]
    spring_type: wp.array  # int32[num_springs]
    spring_unit_stiffness: wp.array  # float32[num_springs]
    spring_unit_damping: wp.array  # float32[num_springs]
    cell_mass: wp.array  # float32[num_cells]
    cell_constraint_offsets: wp.array  # int32[num_cells + 1]
    cell_constraint_indices: wp.array  # int32[num_cells * 28]
    ...
    cell_constraint_offsets_host: np.ndarray
    cell_constraint_indices_host: np.ndarray
    node_support_count_host: np.ndarray
    spring_support_count_host: np.ndarray
    spring_owner_count_host: np.ndarray
    spring_type_host: np.ndarray
    spring_unit_stiffness_host: np.ndarray
    spring_unit_damping_host: np.ndarray
...
@dataclass
class CornerParticleGrid:
    """Result of :func:`build_corner_grid`."""
...
def _constraint_specs(
    ...
    body_ke: float,
    body_kd: float,
) -> tuple[tuple[np.ndarray, int, float, float], ...]:
    return (
        (CELL_EDGE_PAIRS, CONSTRAINT_EDGE, float(edge_ke), float(edge_kd)),
        (CELL_FACE_DIAGONAL_PAIRS, CONSTRAINT_FACE_DIAGONAL, float(face_ke), float(face_kd)),
        (CELL_BODY_DIAGONAL_PAIRS, CONSTRAINT_BODY_DIAGONAL, float(body_ke), float(body_kd)),
    )


def build_corner_grid(
    atlas: DigimouseAtlas,
    edge_ke: float = 1.0e3,
    edge_kd: float = 1.0,
    face_ke: float | None = None,
    face_kd: float | None = None,
    body_ke: float | None = None,
    body_kd: float | None = None,
    ...
    """Build a corner-node particle lattice from a labelled voxel atlas."""
    labels = atlas.labels
    ...
    # Per-cell-pair records, in (cell-major, family-secondary, pair-tertiary)
    # order so cluster id assignment matches the original Python loop exactly.
    constraint_specs = _constraint_specs(
        edge_ke=edge_ke,
        edge_kd=edge_kd,
        face_ke=face_ke,
        face_kd=face_kd,
    ...
    family_i_blocks: list[np.ndarray] = []
    family_j_blocks: list[np.ndarray] = []
    family_id_row: list[np.ndarray] = []
    family_ke_row: list[np.ndarray] = []
    family_kd_row: list[np.ndarray] = []
    for pairs_arr, family_id, unit_ke, unit_kd in constraint_specs:
        a_idx = pairs_arr[:, 0]
        b_idx = pairs_arr[:, 1]
        nodes_a = cell_nodes[:, a_idx]  # (C, P)
        nodes_b = cell_nodes[:, b_idx]
        i_block = np.minimum(nodes_a, nodes_b)
        j_block = np.maximum(nodes_a, nodes_b)
        family_i_blocks.append(i_block)
        family_j_blocks.append(j_block)
        P = pairs_arr.shape[0]
        family_id_row.append(np.full(P, int(family_id), dtype=np.int32))
        family_ke_row.append(np.full(P, float(unit_ke), dtype=np.float32))
        family_kd_row.append(np.full(P, float(unit_kd), dtype=np.float32))

    i_per_cell = np.concatenate(family_i_blocks, axis=1)  # (C, total_per_cell)
    j_per_cell = np.concatenate(family_j_blocks, axis=1)
    pair_family = np.concatenate(family_id_row)  # (total_per_cell,)
    pair_ke = np.concatenate(family_ke_row)
    pair_kd = np.concatenate(family_kd_row)
    total_per_cell = int(i_per_cell.shape[1])

    # Cell-major flatten: record k=cell*total_per_cell+pair runs cells outermost.
    i_flat = i_per_cell.reshape(-1)
    j_flat = j_per_cell.reshape(-1)
    family_flat = np.broadcast_to(pair_family, (num_cells, total_per_cell)).reshape(-1)
    ke_flat = np.broadcast_to(pair_ke, (num_cells, total_per_cell)).reshape(-1)
    kd_flat = np.broadcast_to(pair_kd, (num_cells, total_per_cell)).reshape(-1)

    # Dedupe (i, j) pairs in first-occurrence order.
    keys = (i_flat.astype(np.int64) << 32) | j_flat.astype(np.int64)
    unique_keys, first_record_idx, inverse = np.unique(
        keys, return_index=True, return_inverse=True
    )
    insertion_order = np.argsort(first_record_idx, kind="stable")
    rank = np.empty_like(insertion_order)
    rank[insertion_order] = np.arange(insertion_order.size, dtype=np.int64)
    record_spring_idx = rank[inverse].astype(np.int32, copy=False)
    num_springs = int(unique_keys.size)
    spring_first_record = first_record_idx[insertion_order]

    spring_pairs_np = np.column_stack(
        (i_flat[spring_first_record], j_flat[spring_first_record])
    ).astype(np.int32, copy=False)
    spring_type_np = family_flat[spring_first_record].astype(np.int32, copy=False)
    spring_unit_stiffness_np = ke_flat[spring_first_record].astype(np.float32, copy=False)
    spring_unit_damping_np = kd_flat[spring_first_record].astype(np.float32, copy=False)
    spring_owner_count_np = np.bincount(record_spring_idx, minlength=num_springs).astype(np.int32, copy=False)
    spring_support_count_np = spring_owner_count_np.copy()
    spring_stiffness_np = spring_unit_stiffness_np * spring_support_count_np.astype(np.float32)
    spring_damping_np = spring_unit_damping_np * spring_support_count_np.astype(np.float32)

    # Cell-major record layout means record_spring_idx is already the flat
    # cell-constraint-index buffer with a uniform stride of total_per_cell.
    cell_constraint_indices = record_spring_idx
    cell_constraint_offsets = (
        np.arange(num_cells + 1, dtype=np.int32) * np.int32(total_per_cell)
    )
    ...
    # Bulk-append springs: ModelBuilder.add_spring is a per-call function that
    # re-reads particle_q and runs a numpy norm per spring. With ~1.4M springs at
    # downsample=6 that's >5 s. We replicate its bookkeeping in vectorised form.
    rest_lengths = np.linalg.norm(
        node_positions[spring_pairs_np[:, 0]] - node_positions[spring_pairs_np[:, 1]],
        axis=1,
    ).astype(np.float32, copy=False)
    builder.spring_indices.extend(spring_pairs_np.reshape(-1).tolist())
    builder.spring_stiffness.extend(spring_stiffness_np.tolist())
    builder.spring_damping.extend(spring_damping_np.tolist())
    builder.spring_control.extend([0.0] * num_springs)
    builder.spring_rest_length.extend(rest_lengths.tolist())
    ...
    aux = CornerGridAuxState(
        ...
        spring_support_count=wp.array(spring_support_count_np, dtype=wp.int32, device=device),
        spring_owner_count=wp.array(spring_owner_count_np, dtype=wp.int32, device=device),
        spring_type=wp.array(spring_type_np, dtype=wp.int32, device=device),
        spring_unit_stiffness=wp.array(spring_unit_stiffness_np, dtype=wp.float32, device=device),
        spring_unit_damping=wp.array(spring_unit_damping_np, dtype=wp.float32, device=device),
        cell_mass=wp.array(cell_mass, dtype=wp.float32, device=device),
        cell_constraint_offsets=wp.array(cell_constraint_offsets, dtype=wp.int32, device=device),
        cell_constraint_indices=wp.array(cell_constraint_indices, dtype=wp.int32, device=device),
        grid_shape=(nx, ny, nz),
        ...
        cell_constraint_offsets_host=cell_constraint_offsets,
        cell_constraint_indices_host=cell_constraint_indices,
        node_support_count_host=node_support_count.copy(),
        spring_support_count_host=spring_support_count_np.copy(),
        spring_owner_count_host=spring_owner_count_np.copy(),
        spring_type_host=spring_type_np.copy(),
        spring_unit_stiffness_host=spring_unit_stiffness_np.copy(),
        spring_unit_damping_host=spring_unit_damping_np.copy(),
        node_material_host=node_material.copy(),
        cell_active_host=np.ones(num_cells, dtype=np.int32),
        shape_matching_clusters=None,
    )
    return CornerParticleGrid(model=model, state=state, aux=aux)


def build_corner_shape_matching_clusters(
    pg: CornerParticleGrid,
    ...
    if num_clusters == 0:
        raise ValueError("corner grid has no active cells; cannot build shape-matching clusters")
...
def build_corner_l1_shape_matching_clusters(
    pg: CornerParticleGrid,
    coefficient: float = 1.0,
    device: str | wp.context.Device | None = None,
...
__all__ = [
    "CELL_BODY_DIAGONAL_PAIRS",
    "CELL_CORNER_OFFSETS",
    "CELL_EDGE_PAIRS",
    "CELL_FACE_DIAGONAL_PAIRS",
