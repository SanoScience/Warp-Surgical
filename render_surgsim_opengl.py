    ...
    class SimRenderer(renderer):
        ...
        def log_points(self, name, points, radii=None, colors=None, hidden=False):
            ...
            if isinstance(radii, wp.array):
                radii = radii.numpy()
            self.render_points(name, points, radii, colors)

        def log_mesh(self, name, points, indices, uvs=None, texture=None, hidden=False):
            """Compatibility adapter: stores mesh data for use by log_instances."""
            if not hasattr(self, '_logged_meshes'):
                self._logged_meshes = {}

            diffuse_map_id = None
            if texture is not None and isinstance(texture, np.ndarray):
                if not hasattr(self, '_numpy_texture_ids'):
                    self._numpy_texture_ids = {}
                cache_key = f"_log_mesh_{name}"
                if cache_key not in self._numpy_texture_ids:
                    if not hasattr(self, '_texture_manager'):
                        from texture_loader import OpenGLTextureManager
                        self._texture_manager = OpenGLTextureManager(self)
                    height, width = texture.shape[:2]
                    channels = texture.shape[2] if len(texture.shape) == 3 else 1
                    diffuse_map_id = self._texture_manager._create_gl_texture(
                        texture, width, height, channels
                    )
                    self._numpy_texture_ids[cache_key] = diffuse_map_id
                else:
                    diffuse_map_id = self._numpy_texture_ids[cache_key]

            self._logged_meshes[name] = {
                'points': points,
                'indices': indices,
                'uvs': uvs,
        ...
        def log_instances(self, instance_name, mesh_name, xforms, scales, colors, materials=None):
            """Compatibility adapter: renders a stored mesh via render_mesh_warp."""
            if not hasattr(self, '_logged_meshes') or mesh_name not in self._logged_meshes:
                return

            mesh_data = self._logged_meshes[mesh_name]

            scale_val = scales.numpy()[0] if isinstance(scales, wp.array) else scales
            color_val = colors.numpy()[0] if isinstance(colors, wp.array) else colors

            diffuse_maps = None
            if mesh_data['diffuse_map_id'] is not None:
                diffuse_maps = [mesh_data['diffuse_map_id']]

            self.render_mesh_warp(
                name=instance_name,
                points=mesh_data['points'],
                indices=mesh_data['indices'],
                texture_coords=mesh_data['uvs'],
                diffuse_maps=diffuse_maps,
                scale=tuple(float(v) for v in scale_val),
                basic_color=tuple(float(v) for v in color_val),
