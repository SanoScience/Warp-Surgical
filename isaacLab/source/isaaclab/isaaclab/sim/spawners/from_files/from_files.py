...
def _spawn_from_usd_file(
    ...
    **kwargs,
    ...
    # If a geometric scale was applied, and any prims under this path have physics:density, adjust it by inverse volume scale
    if cfg.scale is not None:
        vol_scale = float(cfg.scale[0]) * float(cfg.scale[1]) * float(cfg.scale[2])
        if vol_scale != 0.0:
            stage = get_current_stage()
            # Traverse whole stage and filter by path prefix for robustness
            ...
            for p in Usd.PrimRange(stage.GetPseudoRoot()):
                ...
                if p_path == prim_path or p_path.startswith(prefix):
                    attr = p.GetAttribute("physics:density")
                    if attr is not None:
                        current = attr.Get()
                        if current is not None:
                            attr.Set(current / vol_scale)
