...
class HapticDevice(object):
    def __init__(self, callback: hd_callback, device_name: str = "Default Device", scheduler_type: str = "async"):
        ...
        # If the current device is the same as the one we just initialized,
        # it means the device was already initialized earlier — bail out.
        if current_id == self.id:
            print("Device {} is already initialized.".format(device_name))
            return

        # Otherwise, make the new device the current one and continue initialization.
