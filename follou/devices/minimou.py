...
class MiniMou(DeviceController):
    ...
    def get_orientation(self) -> tuple:
        """Get orientation as a unit quaternion (x, y, z, w)? in left-handed coordinate system..

        qx = self.get_return(Cmd.GET_ORIENTATION, 0)
        qy = self.get_return(Cmd.GET_ORIENTATION, 2)
        qz = self.get_return(Cmd.GET_ORIENTATION, 1)
        qa = -self.get_return(Cmd.GET_ORIENTATION, 3)
        return (qx, qy, qz, qa)
        """

        """Get orientation as a unit quaternion (x, y, z, w) in right-handed coordinate system.
        
        Converts from device frame to app frame via Z-axis flip
        (basis change P = diag(1, 1, -1), det = -1, handedness-flipping).
        """
        qx = self.get_return(Cmd.GET_ORIENTATION, 0)
