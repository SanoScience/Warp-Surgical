...
"""Corner-grid shape-matching solver tests."""
...
from omnisurg.hex.corner_solver import (
...
def _build_pg(labels: np.ndarray, device: str = "cpu"):
    return build_corner_grid(_atlas_from_labels(labels), device=device)
...
def _reset_cluster_warm_start(solver: SolverCornerShapeMatching) -> None:
...
def _make_solver(pg, **kwargs) -> SolverCornerShapeMatching:
    clusters = build_corner_shape_matching_clusters(pg)
    return SolverCornerShapeMatching(pg.model, clusters, **kwargs)
...
def test_springs_shape_and_mixed_modes_produce_distinct_results():
    ...
    def _run(enable_springs: bool, enable_shape_matching: bool) -> np.ndarray:
        solver = _make_solver(
            pg,
            iterations=6,
    ...
    springs_only = _run(True, False)
    shape_only = _run(False, True)
    mixed = _run(True, True)

    assert np.isfinite(springs_only).all()
    assert np.isfinite(shape_only).all()
    assert np.isfinite(mixed).all()
    assert not np.allclose(springs_only, shape_only)
    assert not np.allclose(mixed, springs_only)
    assert not np.allclose(mixed, shape_only)
...
def test_hierarchical_shape_matching_relaxation_scales_stiffness(
    ...
    def _run(stiffness: float, relaxation: float) -> np.ndarray:
        ...
        solver = SolverCornerShapeMatching(pg.model, clusters, **kwargs)
        state_in = pg.model.state()
        state_out = pg.model.state()
        state_in.particle_q.assign(distorted)
        state_in.particle_qd.zero_()
...
def test_solver_cuda_graph_replay_after_device_cell_deletion_without_recapture():
    ...
    loop_delete = make_corner_deletion_state(pg_loop.model, pg_loop.aux)
    graph_delete = make_corner_deletion_state(pg_graph.model, pg_graph.aux)
...
def test_self_collision_toggle_with_shape_matching_changes_overlapping_clusters():
    ...
    solver_off = SolverCornerShapeMatching(
        pg.model,
        clusters,
        iterations=4,
        enable_springs=False,
    ...
    solver_on = SolverCornerShapeMatching(
        pg.model,
        clusters,
        iterations=4,
        enable_springs=False,
    ...
    def _step_once(solver: SolverCornerShapeMatching) -> np.ndarray:
