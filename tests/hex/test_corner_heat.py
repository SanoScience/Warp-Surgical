...
from omnisurg.hex.corner_grid import build_corner_grid
from omnisurg.hex.corner_heat import make_corner_heat_state
...
from omnisurg.hex._legacy_corner_app import _normalize_instrument_tool_mode
...
def test_diathermy_injection_heats_only_nearby_active_cells():
    pg = build_corner_grid(_line_labels([MUSCLE.id, MUSCLE.id, MUSCLE.id]), device="cpu")
    heat = make_corner_heat_state(pg.model, pg.aux)
...
def test_diathermy_injection_uses_per_sphere_analog_power():
    pg = build_corner_grid(_line_labels([MUSCLE.id]), device="cpu")
    heat = make_corner_heat_state(pg.model, pg.aux)
...
def test_overheated_selection_deletes_only_cuttable_materials():
    pg = build_corner_grid(_line_labels([MUSCLE.id, BONE.id]), device="cpu")
    delete_state = make_corner_deletion_state(pg.model, pg.aux)
    heat = make_corner_heat_state(pg.model, pg.aux)
...
def test_noncuttable_cells_conduct_heat_but_are_not_deleted():
    pg = build_corner_grid(_line_labels([BONE.id, MUSCLE.id]), device="cpu")
    delete_state = make_corner_deletion_state(pg.model, pg.aux)
    heat = make_corner_heat_state(pg.model, pg.aux)
