    ...
    wp_args__enforce_locked_nodes_kernel_3d4c26d0 *_wp_args)
{
    ...
    //---------
    // forward
    // def _enforce_locked_nodes_kernel(                                                      <L 1024>
    // tid = wp.tid()                                                                         <L 1030>
    var_0 = builtin_tid1d();
    // particle_idx = locked_indices[tid]                                                     <L 1031>
    var_1 = wp::address(var_locked_indices, var_0);
    var_3 = wp::load(var_1);
    var_2 = wp::copy(var_3);
    // if particle_idx < 0:                                                                   <L 1032>
    var_5 = (var_2 < var_4);
    if (var_5) {
        // return                                                                             <L 1033>
        return;
    }
    // particle_q[particle_idx] = locked_positions[tid]                                       <L 1035>
    var_6 = wp::address(var_locked_positions, var_0);
    var_7 = wp::load(var_6);
    wp::array_store(var_particle_q, var_2, var_7);
    // particle_qd[particle_idx] = wp.vec3(0.0, 0.0, 0.0)                                     <L 1036>
